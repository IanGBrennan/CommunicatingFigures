...
```{r, eval=F}
setwd("~/Documents/GitHub/CommunicatingFigures/NHM/03_Build_GIF/Natalie_Limerick")
```

Give our GIF file a name
```{r}
gifname <- "Natalie_Limerick"
```
...
Make the animation!
```{r}
list.files(path = paste0(getwd(),"/Natalie_Limerick/"), pattern = "*.jpeg", full.names = T) %>% 
...
knitr::include_graphics("Natalie_Limerick.gif")
```
